# test_supervisor_handoff_router_eval.py
...
JUDGE_PROMPT: str = """
You are grading a supervisor handoff router.
Return JSON with keys score and reason.
score = 1.0 only when the right specialist is selected and unsupported requests fail clearly.
score = 0.0 when requests are misrouted or context leaks between specialists.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/supervisor_handoff_router.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": "refund my invoice", "expected": "billing", "tags": ["billing"]},
        {"input": "rotate this API token", "expected": "security", "tags": ["security"]},
        {"input": "where is the API guide", "expected": "docs", "tags": ["docs"]},
        {"input": "secret leak in invoice flow", "expected": "security", "tags": ["mixed"]},
        {"input": "hello", "expected": "none", "tags": ["unsupported"]},
    ]


def run_router(case_input: str) -> str:
    text = case_input.casefold()
    if any(word in text for word in ["refund", "invoice", "charge"]):
        return "billing"
    if any(word in text for word in ["token", "secret", "permission"]):
        return "security"
    if any(word in text for word in ["docs", "guide", "api"]):
        return "docs"
    return "none"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_router(str(case["input"]))
...
@pytest.mark.parametrize(
    ...
    [
        ("", "none"),
        ("refund then rotate token", "billing"),
        ("A" * 12000, "none"),
    ],
)
