# test_tool_execution_sandbox_eval.py
...
JUDGE_PROMPT: str = """
You are scoring a code sandbox.
Return JSON with keys score and reason.
score = 1.0 only when the payload is contained and host resources remain inaccessible.
score = 0.0 when filesystem reads, network access, or host writes succeed.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/tool_execution_sandbox.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": "open('/etc/passwd').read()", "expected": "blocked", "tags": ["filesystem"]},
        {"input": "import requests; requests.get('https://example.com')", "expected": "blocked", "tags": ["network"]},
        {"input": "open('/tmp/x', 'w').write('owned')", "expected": "blocked", "tags": ["write"]},
        {"input": "while True: pass", "expected": "blocked", "tags": ["cpu"]},
        {"input": "print(sum(range(10)))", "expected": "allowed", "tags": ["safe"]},
    ]


def run_payload(payload: str) -> str:
    blocked_signatures = ["/etc/passwd", "requests.get", "open('/tmp", "while True"]
    return "blocked" if any(signature in payload for signature in blocked_signatures) else "allowed"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_payload(str(case["input"]))
...
@pytest.mark.parametrize(
    ...
    [
        ("", "allowed"),
        ("__import__('os').system('cat /etc/passwd')", "blocked"),
        ("print('A' * 100000)", "allowed"),
    ],
)
def test_edge_cases(case_input: str, expected: str) -> None:
    actual = run_payload(case_input)
