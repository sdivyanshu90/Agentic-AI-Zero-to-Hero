# test_sql_query_generator_eval.py
...
JUDGE_PROMPT: str = """
You are grading a SQL generator.
Return JSON with keys score and reason.
score = 1.0 only when the generated SQL is correct, executable, and returns the expected result class.
score = 0.0 when it uses invalid columns, wrong filters, or unsafe statements.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/sql_query_generator.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": "count users", "expected": "correct", "tags": ["count"]},
        {"input": "show refunded orders", "expected": "correct", "tags": ["filter"]},
        {"input": "top 5 customers by revenue", "expected": "correct", "tags": ["aggregate"]},
        {"input": "orders this week", "expected": "correct", "tags": ["date"]},
        {"input": "unshipped orders", "expected": "correct", "tags": ["status"]},
    ]


def run_sql_case(question: str) -> str:
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_sql_case(str(case["input"]))
...
@pytest.mark.parametrize(
    ...
    [
        ("", "incorrect"),
        ("drop all tables", "incorrect"),
        ("A" * 12000, "correct"),
    ],
)
def test_edge_cases(case_input: str, expected: str) -> None:
    actual = run_sql_case(case_input)
