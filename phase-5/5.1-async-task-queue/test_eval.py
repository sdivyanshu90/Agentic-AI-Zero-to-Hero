...
JUDGE_PROMPT: str = """
You are grading an async task queue.
Return JSON with keys score and reason.
score = 1.0 only when the task reaches the expected terminal or intermediate state without skipping retries or DLQ behavior.
score = 0.0 when the queue loses the task, over-retries, or returns the wrong status.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/async_task_queue.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": {"prompt": "summarize logs", "failures": 0}, "expected": "Completed", "tags": ["happy_path"]},
        {"input": {"prompt": "retry once", "failures": 1}, "expected": "Completed", "tags": ["retry"]},
        {"input": {"prompt": "retry thrice then fail", "failures": 3}, "expected": "Failed", "tags": ["dlq"]},
        {"input": {"prompt": "small task", "failures": 0}, "expected": "Completed", "tags": ["small"]},
        {"input": {"prompt": "transient timeout", "failures": 2}, "expected": "Completed", "tags": ["transient"]},
    ]


def run_queue_simulation(payload: dict[str, Any]) -> str:
    failures = int(payload.get("failures", 0))
    return "Failed" if failures >= 3 else "Completed"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_queue_simulation(dict(case["input"]))
...
@pytest.mark.parametrize(
    ("case_input", "expected"),
    [
        ({"prompt": "", "failures": 0}, "Completed"),
        ({"prompt": "A" * 20000, "failures": 0}, "Completed"),
        ({"prompt": "force failure", "failures": 4}, "Failed"),
    ],
)
def test_edge_cases(case_input: dict[str, Any], expected: str) -> None:
    actual = run_queue_simulation(case_input)
