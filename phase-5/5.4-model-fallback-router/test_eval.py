# test_model_fallback_router_eval.py
...
JUDGE_PROMPT: str = """
You are grading a provider fallback router.
Return JSON with keys score and reason.
score = 1.0 only when fallback happens on primary failure and shared budget shedding prevents cascade failure.
score = 0.0 when the caller sees an avoidable error or both providers are hammered unnecessarily.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/model_fallback_router.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": {"primary": "ok", "secondary": "unused", "budget": 2}, "expected": "served", "tags": ["primary_ok"]},
        {"input": {"primary": "fail", "secondary": "ok", "budget": 2}, "expected": "fallback", "tags": ["fallback"]},
        {"input": {"primary": "fail", "secondary": "fail", "budget": 2}, "expected": "shed", "tags": ["dual_failure"]},
        {"input": {"primary": "ok", "secondary": "unused", "budget": 0}, "expected": "shed", "tags": ["budget_exhausted"]},
        {"input": {"primary": "fail", "secondary": "ok", "budget": 1}, "expected": "fallback", "tags": ["last_budget"]},
    ]


def run_router(case_input: dict[str, Any]) -> str:
    if int(case_input["budget"]) <= 0:
        return "shed"
    if case_input["primary"] == "ok":
        return "served"
    if case_input["secondary"] == "ok":
        return "fallback"
    return "shed"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_router(dict(case["input"]))
...
@pytest.mark.parametrize(
    ("case_input", "expected"),
    [
        ({"primary": "ok", "secondary": "unused", "budget": 0}, "shed"),
        ({"primary": "fail", "secondary": "fail", "budget": 5}, "shed"),
        ({"primary": "fail", "secondary": "ok", "budget": 100}, "fallback"),
    ],
)
def test_edge_cases(case_input: dict[str, Any], expected: str) -> None:
    actual = run_router(case_input)
