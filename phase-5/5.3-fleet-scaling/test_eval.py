...
JUDGE_PROMPT: str = """
You are grading a fleet scaling rate limiter.
Return JSON with keys score and reason.
score = 1.0 only when the limiter delays or sheds before provider 429s occur.
score = 0.0 when the limiter allows overload or sheds too early.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/fleet_scaling.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": {"window_count": 1000}, "expected": "allowed", "tags": ["low_load"]},
        {"input": {"window_count": 2600}, "expected": "delayed", "tags": ["approaching_limit"]},
        {"input": {"window_count": 2999}, "expected": "delayed", "tags": ["edge_limit"]},
        {"input": {"window_count": 3001}, "expected": "shed", "tags": ["over_limit"]},
        {"input": {"window_count": 4500}, "expected": "shed", "tags": ["attack"]},
    ]


def run_limiter(case_input: dict[str, Any]) -> str:
    window_count = int(case_input["window_count"])
    if window_count > 3000:
        return "shed"
    if window_count > 2550:
        return "delayed"
    return "allowed"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_limiter(dict(case["input"]))
...
@pytest.mark.parametrize(
    ("case_input", "expected"),
    [
        ({"window_count": 0}, "allowed"),
        ({"window_count": 100000}, "shed"),
        ({"window_count": 2700}, "delayed"),
    ],
)
def test_edge_cases(case_input: dict[str, Any], expected: str) -> None:
    actual = run_limiter(case_input)
