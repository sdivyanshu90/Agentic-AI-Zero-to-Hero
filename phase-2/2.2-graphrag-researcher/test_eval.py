# test_graphrag_researcher_eval.py
...
JUDGE_PROMPT: str = """
You are grading GraphRAG answers.
Return JSON with keys score and reason.
score = 1.0 only when the answer reflects the correct multi-hop graph traversal and does not invent edges.
score = 0.0 when the path is wrong or unsupported.
""".strip()
...
def golden_dataset() -> list[dict[str, Any]]:
    dataset_path = Path("golden/graphrag_researcher.json")
    if dataset_path.exists():
        return json.loads(dataset_path.read_text(encoding="utf-8"))
    return [
        {"input": "Which investors funded the acquirer of startup X?", "expected": "correct", "tags": ["two_hop"]},
        {"input": "Who acquired company Y?", "expected": "correct", "tags": ["one_hop"]},
        {"input": "Which company built X?", "expected": "correct", "tags": ["builder"]},
        {"input": "Which investors funded company Z?", "expected": "correct", "tags": ["funding"]},
        {"input": "Who is connected to startup A through acquisition?", "expected": "correct", "tags": ["path"]},
    ]


def run_graph_case(question: str) -> str:
    return "correct" if question else "incorrect"
...
def test_eval_threshold(golden_dataset: list[dict[str, Any]]) -> None:
    ...
    for case in golden_dataset:
        actual = run_graph_case(str(case["input"]))
...
@pytest.mark.parametrize(
    ...
    [
        ("", "incorrect"),
        ("Use relation OWNS instead of ACQUIRED", "incorrect"),
        ("A" * 12000, "correct"),
    ],
)
def test_edge_cases(case_input: str, expected: str) -> None:
    actual = run_graph_case(case_input)
